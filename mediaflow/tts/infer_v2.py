...
from mediaflow.tts.gpt.model_v2 import UnifiedVoice
from mediaflow.tts.utils.maskgct_utils import build_semantic_model, build_semantic_codec
from mediaflow.tts.utils.checkpoint import load_checkpoint
from mediaflow.tts.utils.front import TextNormalizer, TextTokenizer

from mediaflow.tts.s2mel.modules.commons import load_checkpoint2, MyModel
from mediaflow.tts.s2mel.modules.bigvgan import bigvgan
from mediaflow.tts.s2mel.modules.campplus.DTDNN import CAMPPlus
from mediaflow.tts.s2mel.modules.audio import mel_spectrogram
...
class IndexTTS2:
    def __init__(
        ...
        if self.use_cuda_kernel:
            ...
            try:
                from mediaflow.tts.s2mel.modules.bigvgan.alias_free_activation.cuda import activation1d
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        # 濡傛灉鍙傝€冮煶棰戞敼鍙樹簡锛屾墠闇€瑕侀噸鏂扮敓鎴? 鎻愬崌閫熷害
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt:
            ...
            feat = feat - feat.mean(dim=0, keepdim=True)  # feat2鍙﹀涓€涓护娉㈠櫒鑳介噺缁勭壒寰乕922, 80]
            style = self.campplus_model(feat.unsqueeze(0))  # 鍙傝€冮煶棰戠殑鍏ㄥ眬style2[1,192]
...
class QwenEmotion:
    def __init__(self, model_dir):
        ...
        self.cn_key_to_en = {
            ...
            "鎭愭儳": "afraid",
            "鍙嶆劅": "disgusted",
            # TODO: the "浣庤惤" (melancholic) emotion will always be mapped to
            # "鎮蹭激" (sad) by QwenEmotion's text analysis. it doesn't know the
        ...
        self.desired_vector_order = ["楂樺叴", "鎰ゆ€?, "鎮蹭激", "鎭愭儳", "鍙嶆劅", "浣庤惤", "鎯婅", "鑷劧"]
        self.melancholic_words = {
            # emotion text phrases that will force QwenEmotion's "鎮蹭激" (sad) detection
            # to become "浣庤惤" (melancholic) instead, to fix limitations mentioned above.
            "浣庤惤",
            "melancholy",
            "melancholic",
            "depression",
    ...
    def inference(self, text_input):
        ...
        # workaround for QwenEmotion's inability to distinguish "鎮蹭激" (sad) vs "浣庤惤" (melancholic).
        ...
        if any(word in text_input_lower for word in self.melancholic_words):
            ...
            content["鎮蹭激"], content["浣庤惤"] = content.get("浣庤惤", 0.0), content.get("鎮蹭激", 0.0)
...
if __name__ == "__main__":
    ...
    text = '娆㈣繋澶у鏉ヤ綋楠宨ndextts2锛屽苟缁欎簣鎴戜滑鎰忚涓庡弽棣堬紝璋㈣阿澶у銆?
    tts = IndexTTS2(
