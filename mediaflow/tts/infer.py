...
from mediaflow.tts.BigVGAN.models import BigVGAN as Generator
from mediaflow.tts.gpt.model import UnifiedVoice
from mediaflow.tts.utils.checkpoint import load_checkpoint
from mediaflow.tts.utils.feature_extractors import MelSpectrogramFeatures
...
class IndexTTS:
    def __init__(
        ...
        #
        # from mediaflow.tts.vqvae.xtts_dvae import DiscreteVAE
        ...
        if self.use_fp16:
            try:
                ...
                use_deepspeed = False
                print(f">> DeepSpeed鍔犺浇澶辫触锛屽洖閫€鍒版爣鍑嗘帹鐞? {e}")
        ...
        if self.use_cuda_kernel:
            ...
            try:
                from mediaflow.tts.BigVGAN.alias_free_activation.cuda import load
    ...
    def pad_tokens_cat(self, tokens: List[torch.Tensor]) -> torch.Tensor:
        if self.model_version and self.model_version >= 1.5:
            # 1.5鐗堟湰浠ヤ笂锛岀洿鎺ヤ娇鐢╯top_text_token 鍙充晶濉厖锛屽～鍏呭埌鏈€澶ч暱搴?
            # [1, N] -> [N,]
    ...
    # 蹇€熸帹鐞嗭細瀵逛簬鈥滃鍙ラ暱鏂囨湰鈥濓紝鍙疄鐜拌嚦灏?2~10 鍊嶄互涓婄殑閫熷害鎻愬崌~ 锛團irst modified by sunnyboxs 2025-04-16锛?
    def infer_fast(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_segment=100,
        ...
        """
        Args:
            ``max_text_tokens_per_segment``: 鍒嗗彞鐨勬渶澶oken鏁帮紝榛樿``100``锛屽彲浠ユ牴鎹瓽PU纭欢鎯呭喌璋冩暣
                - 瓒婂皬锛宐atch 瓒婂锛屾帹鐞嗛€熷害瓒?蹇?锛屽崰鐢ㄥ唴瀛樻洿澶氾紝鍙兘褰卞搷璐ㄩ噺
                - 瓒婂ぇ锛宐atch 瓒婂皯锛屾帹鐞嗛€熷害瓒?鎱?锛屽崰鐢ㄥ唴瀛樺拰璐ㄩ噺鏇存帴杩戜簬闈炲揩閫熸帹鐞?
            ``segments_bucket_max_size``: 鍒嗗彞鍒嗘《鐨勬渶澶у閲忥紝榛樿``4``锛屽彲浠ユ牴鎹瓽PU鍐呭瓨璋冩暣
                - 瓒婂ぇ锛宐ucket鏁伴噺瓒婂皯锛宐atch瓒婂锛屾帹鐞嗛€熷害瓒?蹇?锛屽崰鐢ㄥ唴瀛樻洿澶氾紝鍙兘褰卞搷璐ㄩ噺
                - 瓒婂皬锛宐ucket鏁伴噺瓒婂锛宐atch瓒婂皯锛屾帹鐞嗛€熷害瓒?鎱?锛屽崰鐢ㄥ唴瀛樺拰璐ㄩ噺鏇存帴杩戜簬闈炲揩閫熸帹鐞?
        """
        ...
        # 濡傛灉鍙傝€冮煶棰戞敼鍙樹簡锛屾墠闇€瑕侀噸鏂扮敓鎴?cond_mel, 鎻愬崌閫熷害
        ...
        # clear cache
        tqdm_progress.close()  # 纭繚杩涘害鏉¤鍏抽棴
