...
import transformers
from transformers import GPT2Config, GPT2PreTrainedModel, GPT2Model, LogitsProcessorList
from transformers.generation import GenerationMixin
from transformers.modeling_outputs import CausalLMOutputWithCrossAttentions

from mediaflow.tts.gpt._compat import _LocalModelParallel
from mediaflow.tts.gpt.conformer_encoder import ConformerEncoder
from mediaflow.tts.gpt.perceiver import PerceiverResampler
from mediaflow.tts.utils.arch_util import AttentionBlock
from mediaflow.tts.utils.typical_sampling import TypicalLogitsWarper
...
class GPT2InferenceModel(GPT2PreTrainedModel, _LocalModelParallel):
    def __init__(self, config, gpt, text_pos_emb, embeddings, norm, linear, kv_cache=False):
        super().__init__(config)
        _LocalModelParallel.__init__(self)
    ...
    def parallelize(self, device_map=None):
        _LocalModelParallel.parallelize(self, device_map)
        if hasattr(self.transformer, "parallelize"):
            self.transformer.parallelize(device_map)
        if hasattr(self.transformer, "first_device"):
            self.lm_head = self.lm_head.to(self.transformer.first_device)

    def deparallelize(self):
        if hasattr(self.transformer, "deparallelize"):
            self.transformer.deparallelize()
        self.transformer = self.transformer.to("cpu")
        self.lm_head = self.lm_head.to("cpu")
        _LocalModelParallel.deparallelize(self)
...
class UnifiedVoice(nn.Module):
    ...
    def post_init_gpt2_config(self, use_deepspeed=False, kv_cache=False, half=False):
        ...
        if self.use_accel and torch.cuda.is_available():
            ...
            from mediaflow.tts.accel import GPT2AccelModel, AccelInferenceEngine
